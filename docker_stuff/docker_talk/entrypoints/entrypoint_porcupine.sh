...
export YARP_DATA_DIRS=$YARP_DATA_DIRS:${ROBOT_CODE}/.config/porcupine

cd ${ROBOT_CODE}/tour-guide-robot/ && rm -rf build && mkdir build && \
    (git fetch ${tourRemote} || (git remote add ${tourRemote} https://github.com/${tourRemote}/tour-guide-robot && git fetch ${tourRemote})) && git checkout ${tourBranch} && git pull && cd build && cmake .. -DENABLE_faceExpressionImage=ON -DSILERO_VAD=ON -DWAKE_WORD=ON && make -j11
